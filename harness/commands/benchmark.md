...
| Override inner timing | `BENCHMARK_REP=20 BENCHMARK_WARMUP=5 bash harness/harness/benchmark/run_benchmark.sh` |
| Override SDPA timing | `BENCHMARK_SDPA_REP=20 BENCHMARK_SDPA_WARMUP=5 bash harness/harness/benchmark/run_benchmark.sh` |

## Fixed Benchmark Command

```bash
python3 harness/harness/benchmark/bench_sm100_hd256.py --suite gate --rep 50 --warmup 10
```
...
FA rows are repeated by `BENCHMARK_RUNS` and compared by median. PyTorch SDPA
baseline is run once separately over the same logical workload and written to
`sdpa_baseline.log`. Reports include FA median TFLOPS, SDPA TFLOPS, `FA/SDPA`,
and `Gap vs SDPA`. `Gap vs SDPA = FA / SDPA - 1`; positive means FA is faster,
negative means FA trails SDPA.
...
| `harness/harness/benchmark/compare_benchmark.py` | Parses suite-aware dense/varlen logs, compares current vs previous medians. |
